...
for (var i = 1; i < 101; i++) { createH3(i); };
function createH3(count) {
    var h3 = document.createElement('h3').setAttribute("id", count);
    h3.setAttribute("id", count);
    h3.innerHTML = `Accusation ${count}`;
    document.body.appendChild(h3);
    h3.addEventListener('click', () => alert("I accuse " + friends[i % friends.length] + "with the " + weapons[i % weapons.length] + "in the " + places[i % places.length]));};